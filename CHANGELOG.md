...
- New **live-poll** example app — the no-build proof: a complete kerf app served as authored source (plain `main.js` + an `index.html` importmap resolving `kerfjs` to static files; no JSX, no bundler, no transform — view-source shows the app). Authored entirely with the `html` tagged template, and a fully-bound mount: the render reads no signal `.value`, so it runs exactly once while every vote flows through fine-grained bindings (bound counts, bound bar `style`, bound total; a "renders" badge stays at 1). Ships with a three-engine browser smoke spec, an animated demo capture, and a design doc (`docs/15-no-build-example.md`) specifying the vendor-copy serving contract.
...
- Runtime dev-mode override: set `globalThis.KERF_DEV = false` (or `true`) before mounting to force production (or development) behavior regardless of `NODE_ENV`. This fixes no-bundler / CDN-importmap consumers — which have no `process` — being permanently stuck in development mode (store `get()` freeze, opt-in warnings) in their production deployment. With no override set, behavior is unchanged: bundled consumers follow `NODE_ENV` and bundlers still dead-code-eliminate the dev paths.

- New opt-in dev warning `KERF_DEV_WARN_VALUE_ONLY_RERENDER=1`: flags a `mount()` re-render whose only differences from the previous render were text content and attribute values — no structural change — meaning every changed hole could have been a fine-grained binding ("values bind, structure re-renders": pass the signal/computed itself and each change updates just that node; a mount whose render reads no `.value` never re-renders at all). Fires once per mount, detects via a conservative lockstep tree compare (any shape change stays silent), and is free in production and when not opted in, like the rest of the dev-warn family.
...
- JSX typing: `<filter>` is now a typed intrinsic element (`SVGCommonAttrs`), matching the SVG fragment set `toElement()` supports — previously `toElement('<filter>…')` worked while JSX-authored `<filter>` failed to compile.
- Better errors for two consumer mistakes that previously died with bare engine TypeErrors: `morph(null, template)` now throws the same descriptive getElementById-typo hint as `mount(null, …)`, and an `each()` list *introduced* inside a `data-morph-skip` subtree on a re-render (whose marker therefore never reaches the live DOM) now throws an error naming the cause and the fix instead of failing inside the reconciler.
- The `KERF_DEV_WARN_STALE_BINDING` warner now routes its dev-mode decision through the shared `isDevMode()` gate like the rest of the dev-warn family, so the `globalThis.KERF_DEV` boolean override governs it (previously it read `NODE_ENV` directly and ignored the override).
- **Bound holes reading a row's own data now survive in-place updates.** Previously, when an `arraySignal.update()` (or a `cacheKey`-driven re-render) applied to a row in place — the DOM node reused via the surgical fast paths or the in-place morph — the old binding effects were carried forward and the fresh render's bindings discarded, so a hole like `{computed(() => item.label)}` kept reading the pre-update row object and silently went stale (a documented spike limitation). In-place updates now compare binding instances per hole: unchanged instances (stable external signals, cache hits — e.g. the select-row pattern) carry forward with zero extra work, and changed instances are disposed and re-wired against the surviving node. Also fixed along the way: a tag-changed row on the snapshot in-place path now gets its fresh bindings wired (they were previously dropped silently), and re-wiring a text hole reuses its marker's existing text node instead of stacking a duplicate.
