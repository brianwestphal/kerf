...
- Fixed `TokenSearchField` first-line alignment so the leading icon, editable text, clear action, and trailing content share the field's vertical center while remaining pinned when text wraps. The UX catalog now also demonstrates a responsive toolbar find field that collapses to a search action at narrow widths and moves focus into the expanded editor.
...
- Fixed documentation sidebar and table-of-contents labels collapsing into the 18px icon column when the site combines the published `@kerfjs/ui` MenuItem markup with the newer shared sidebar composition. Iconless navigation now owns the full label track across desktop, tablet, and mobile drawers, with Playwright regression coverage for readable label width.
- Added a full-site Playwright visual audit covering all 75 emitted HTML surfaces at desktop, tablet, and mobile widths, including documentation, isolated example apps, demos, redirects, and the 404 page, with 225 full-page captures plus overflow, broken-image, and collapsed-content checks. The audit fixed narrow-screen overflow in long inline API identifiers, the dashboard chart, and the Kanban board; the Kanban, host-selector, and Markdown-editor apps now use deliberate stacked phone layouts while preformatted code blocks retain their own scrolling.
...
- Added eight lazy, production-backed `@kerfjs/ui` composition recipes for application shells, sidebars, workspace headers, master-detail dialogs, composer forms, list-state lifecycles, mixed-control toolbars, and command palettes, with stable catalog routes, canonical AI metadata, drift gates, responsive/keyboard/state coverage, and a copyable stable-root adapter that wires delegated actions, form/dialog events, resize commits, and idempotent disposal outside the catalog.
...
- Refined the Kerf-rendered documentation shell: iconless primary navigation no longer reserves an empty icon column, while the site toolbar now uses `ToolbarControlGroup` with named native navigation, history, appearance, and search actions. Search fills the remaining toolbar width, yields its shortcut hint and then the wordmark as space tightens, and keeps every command reachable through desktop, tablet, mobile, 320px, and 200%-reflow layouts.
