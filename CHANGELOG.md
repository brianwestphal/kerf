...
- Docs: add a `/kerf/migrating/` comparison hub with one page per source framework (React, Alpine, Lit, vanjs). KF-132 ships the index (comparison matrix + perf snapshot), the four per-framework page stubs, the sidebar `Migrating` section, and a `Coming from React?` hero CTA on the homepage; full side-by-side translations land under KF-132a/b/c/d. New requirements doc at `docs/10-migrating.md` (KF-132).
