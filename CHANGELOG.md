...
- Aligned the remaining bordered/padded Web Awesome chrome in `@kerfjs/ui/webawesome.css` to two explicit Kerf inset tiers: `--kui-wa-control-inset` (8px) now drives tabs, inset tree selection, tags, and dropdown items, while `--kui-wa-container-inset` (16px) drives Card, Callout, Details, Accordion Item, and Tab Panel content. Buttons already inherit the 8px form-control inset; compact badges, unframed Breadcrumb, and slotted Scroller item chrome deliberately retain their native geometry. Both tiers are overridable by scope and are pinned by three-engine computed-style coverage plus light/dark wide and narrow visual QA.
