...
describe('release package preparation', () => {
  ...
  it('builds a project generated by the packed initializer with TypeScript 6', () => {
    ...
    execFileSync('tar', ['-xf', tarballs.get('create-kerf-component')!, '-C', artifactRoot]);
    execFileSync(process.execPath, [join(artifactRoot, 'package/index.js'), generatedRoot]);
    ...
    symlinkSync(join(repoRoot, 'node_modules'), join(generatedRoot, 'node_modules'), 'dir');
    const typescriptPackage = JSON.parse(
      readFileSync(join(generatedRoot, 'node_modules/typescript/package.json'), 'utf8'),
    );
    ...
    expect(readFileSync(join(generatedRoot, 'dist/counter.d.ts'), 'utf8')).toContain(
      'createCounter',
    );
    expect(readFileSync(join(generatedRoot, 'dist/counter.js'), 'utf8')).toContain(
      'createCounter',
    );
  }, 20_000);
