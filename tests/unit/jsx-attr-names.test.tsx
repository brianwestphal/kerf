...
function declaredAttributeKeys(): Set<string> {
  ...
  for (const [, key] of TYPES_SRC.matchAll(
...
describe('JSX attribute name emission', () => {
  ...
  it('the `default*` family renders the real attribute, not its own name', () => {
    ...
    expect(String(<input defaultValue="x" defaultChecked />)).toBe(
      '<input value="x" checked>',
    );
  });
  ...
  it('no alias row is dead — every alias key is a declared attribute', () => {
    ...
    const dead = Object.keys(ATTR_ALIASES).filter(
      (k) => !declared.has(k) && !typedElsewhere.has(k),
    );
    expect(dead).toEqual([]);
  });
});
