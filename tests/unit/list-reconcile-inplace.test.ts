...
describe('snapshot in-place content-update fast path', () => {
          ...
          children: each(
            ...
            (it: { id: number }) =>
              jsx('tr', {
                'data-key': String(it.id),
                children: jsx('td', {
                  children: it.id === 1 ? label.value : 'x',
                }),
              }),
  ...
  it('no-content-change re-render keeps every node (all rows unchanged branch)', () => {
    ...
    mount(root, () => {
      ...
      return jsx('tbody', {
        children: each(items, (it: { id: number }) =>
          jsx('tr', {
            'data-key': String(it.id),
            children: jsx('td', { children: String(it.id) }),
          }),
  ...
  it('does NOT fire when a row is removed (length change → main path)', () => {
    ...
    mount(root, () =>
      jsx('tbody', {
        children: each(list.value, (it: { id: number }) =>
          jsx('tr', {
            'data-key': String(it.id),
            children: jsx('td', { children: String(it.id) }),
          }),
    ...
    const ids = Array.from(root.querySelectorAll('tr')).map((tr) =>
  ...
  it('a row that both moves AND changes content takes the main (replace) path', () => {
    ...
    const liB = Array.from(root.querySelectorAll('li')).find(
      (l) => l.getAttribute('data-key') === '2',
    )!;
    ...
    const order = Array.from(root.querySelectorAll('li')).map((l) =>
      l.getAttribute('data-key'),
    );
    expect(order).toEqual(['2', '1']);
    const liBafter = Array.from(root.querySelectorAll('li')).find(
      (l) => l.getAttribute('data-key') === '2',
    )!;
