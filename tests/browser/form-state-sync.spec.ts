...
test('controlled checkbox re-checks after user uncheck (dirty-checked flag)', async ({
  page,
}) => {
  ...
  const result = await page.evaluate(() => {
    ...
    mount(root, () =>
      jsx('div', {
        children: jsx('input', { type: 'checkbox', checked: on.value }),
      }),
    );
...
test('controlled checkbox un-checks after user check (dirty-checked flag)', async ({
  page,
}) => {
...
test('uncontrolled checkbox keeps user state across unrelated re-renders', async ({
  page,
}) => {
  ...
  const result = await page.evaluate(() => {
    ...
    mount(root, () =>
      jsx('div', {
        children: [jsx('input', { type: 'checkbox' }), label.value],
      }),
    );
    const box = root.querySelector('input') as HTMLInputElement;
...
test('controlled value updates a non-focused input the user typed into (dirty-value flag)', async ({
  page,
}) => {
  const result = await page.evaluate(() => {
    const { mount, signal } = (window as any).kerf;
...
test('focused input keeps in-progress typing (preservation rule unchanged)', async ({
  page,
}) => {
  ...
  const result = await page.evaluate(() => {
    ...
    mount(root, () =>
      jsx('div', {
        children: [jsx('input', { type: 'text', value: v.value }), label.value],
      }),
    );
    const input = root.querySelector('input') as HTMLInputElement;
    input.focus();
...
test('controlled <select> follows selected= after the user picked another option', async ({
  page,
}) => {
  const result = await page.evaluate(() => {
    const { mount, signal } = (window as any).kerf;
      ...
      jsx('div', {
        children: jsx('select', {
          children: [
            jsx('option', {
              value: 'a',
              selected: sel.value === 'a',
              children: 'A',
            }),
            jsx('option', {
              value: 'b',
              selected: sel.value === 'b',
              children: 'B',
            }),
          ],
        }),
...
test('controlled <textarea> follows template text after user typing (dirty-value flag)', async ({
  page,
}) => {
  const result = await page.evaluate(() => {
    const { mount, signal } = (window as any).kerf;
...
test('uncontrolled-ish <textarea> keeps user typing across unrelated re-renders', async ({
  page,
}) => {
  ...
  const result = await page.evaluate(() => {
    ...
    mount(root, () =>
      jsx('div', {
        children: [jsx('textarea', { children: 'fixed' }), label.value],
      }),
    );
    const ta = root.querySelector('textarea') as HTMLTextAreaElement;
    ta.value = 'typed';
...
test('fine-grained bound checked={sig} syncs the property too', async ({
  page,
}) => {
