...
6. **Check for anti-patterns documented in CLAUDE.md and the design docs**
   ...
   - **Dependency creep**. CLAUDE.md says: _"No transitive deps beyond `@preact/signals-core`."_ (The former `morphdom` dependency was removed — kerf's reconciler is now `src/morph.ts`.) Open `package.json` and verify the `dependencies` block has only `@preact/signals-core`. Anything else is a violation; flag it.
