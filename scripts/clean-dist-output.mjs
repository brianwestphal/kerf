...
const BARE_CHUNK_IMPORT =
  /^(\s*)import\s+(['"])(\.\/chunk-[^'"]+\.js)\2;(?=\r?$)/gm;
const STATIC_CHUNK_REFERENCE =
  /\b(?:from\s+|import\s*)(['"])(\.\/chunk-[^'"]+\.js)\1/g;
...
async function clean(directory) {
  ...
  for (const [file, source] of sources) {
    ...
    graph.set(
      key,
      chunkReferences(source).map(
        (reference) => `./${relative(root, resolve(dirname(file), reference))}`,
      ),
    );
  }

  for (const [file, original] of sources) {
    const roots = nonBareChunkReferences(original).map(
      (reference) => `./${relative(root, resolve(dirname(file), reference))}`,
    );
    let source = original.replace(
      BARE_CHUNK_IMPORT,
      (statement, _space, _quote, reference) => {
        const target = `./${relative(root, resolve(dirname(file), reference))}`;
        if (!isReachable(target, roots, graph)) {
          throw new Error(
            `${relative(process.cwd(), file)} has a non-redundant bare chunk import: ${statement.trim()}`,
          );
        }
    ...
    if (directives.length > 1) {
      ...
      if (names.size !== 1) {
        throw new Error(
          `${relative(process.cwd(), file)} has conflicting source-map directives`,
        );
      }
      let remaining = directives.length - 1;
      source = source.replace(SOURCE_MAP_DIRECTIVE, (directive) =>
        remaining-- > 0 ? blankMatch(directive) : directive,
      );
    }
  ...
  console.log(
    `Cleaned ${files.length} JavaScript outputs in ${basename(root)}/`,
  );
}

const directory = process.argv[2];
if (!directory)
  throw new Error('Usage: node scripts/clean-dist-output.mjs <dist-directory>');
