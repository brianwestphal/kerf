...
function replace(relative, pattern, replacement) {
  ...
  if (after === before && !pattern.test(before)) {
    throw new Error(
      `[sync-lockstep-versions] could not find the expected version surface in ${relative}`,
    );
  }
...
if (!Number.isInteger(major))
  throw new Error(`[sync-lockstep-versions] invalid root version: ${version}`);

const peerRange = `^${major}.0.0`;
const devRange = `^${version}`;
const docs = [
  'docs/13-component-packages.md',
...
if (write) {
  ...
  for (const relative of docs) {
    replace(
      relative,
      /"peerDependencies": \{ "kerfjs": "[^"]+" \}/,
      `"peerDependencies": { "kerfjs": "${peerRange}" }`,
    );
    replace(
      relative,
      /"devDependencies": \{ "kerfjs": "[^"]+",/,
      `"devDependencies": { "kerfjs": "${devRange}",`,
    );
  }
  ...
  for (const [relative, linkKey] of linkedRootLocks) {
    ...
    if (!linkedRoot)
      throw new Error(
        `[sync-lockstep-versions] missing linked root ${linkKey} in ${relative}`,
      );
...
if (
  ...
  errors.push(
    `package-lock.json does not agree with package.json version ${version}`,
  );
}
const companions = [
  ...
  [
    ...
    'create-kerf-component/package-lock.json',
  ],
...
for (const [manifestPath, lockPath] of companions) {
  ...
  if (manifest.version !== version)
    errors.push(`${manifestPath} is ${manifest.version}; expected ${version}`);
  if (lock.version !== version)
    errors.push(`${lockPath} is ${lock.version}; expected ${version}`);
  if (lock.packages?.['']?.version !== version)
    errors.push(
      `${lockPath} root is ${lock.packages?.['']?.version}; expected ${version}`,
    );
}

for (const [relative, linkKey] of linkedRootLocks) {
  const linkedRoot = readJson(relative).packages?.[linkKey];
  if (linkedRoot?.version !== version)
    errors.push(
      `${relative} linked root is ${linkedRoot?.version}; expected ${version}`,
    );
  for (const field of ['dependencies', 'devDependencies', 'engines']) {
    if (
      JSON.stringify(linkedRoot?.[field]) !== JSON.stringify(rootPackage[field])
    ) {
      errors.push(
        `${relative} linked root ${field} does not match package.json`,
      );
    }
  }
}
...
const pluginMeta = pluginSource.match(
  /meta:\s*\{\s*name:\s*["']eslint-plugin-kerfjs["'],\s*version:\s*["']([^"']+)["']\s*\}/,
)?.[1];
if (pluginMeta !== version)
  errors.push(
    `eslint-plugin/index.js meta.version is ${pluginMeta ?? 'missing'}; expected ${version}`,
  );
...
if (lockEslintPeer !== manifestEslintPeer) {
  errors.push(
    `eslint-plugin/package-lock.json peer is ${lockEslintPeer}; expected ${manifestEslintPeer}`,
  );
}
...
if (template.peerDependencies?.kerfjs !== peerRange) {
  errors.push(
    `component template peer is ${template.peerDependencies?.kerfjs}; expected ${peerRange}`,
  );
}
...
for (const relative of docs) {
  ...
  if (!text.includes(`"peerDependencies": { "kerfjs": "${peerRange}" }`)) {
    errors.push(
      `${relative} does not show the current-major peer range ${peerRange}`,
    );
  }
  if (!text.includes(`"devDependencies": { "kerfjs": "${devRange}",`)) {
    errors.push(
      `${relative} does not show the current release dev range ${devRange}`,
    );
  }
}
...
if (errors.length > 0) {
  throw new Error(
    `[sync-lockstep-versions] version drift:\n- ${errors.join('\n- ')}\nRun npm run sync:lockstep-versions after intentional version changes.`,
  );
}

console.log(
  `[sync-lockstep-versions] OK — four packages and companion metadata agree on ${version}`,
);
