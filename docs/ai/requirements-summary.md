...
| §14 | Feature coverage (per-behavior index + `check:features` gate) | Partial — the gate ships, but export completeness omits `kerfjs/router` and the complete-app browser row represents only Kanban rather than all eleven apps |
