...
| §21 | `@kerfjs/ui` first-party component package | Shipped — Hot Sheet 2-compatible overridable semantic palettes, canonical responsive application-spacing and scroll-owner roles, generic SafeHtml primitives including sibling-action MenuActionRow, controlled SegmentedControl, and reorderable TabBar, component-reachable tree-shakable browser CSS plus CSS-free root/SSR entries, explicit Web Awesome registration, accessibility contracts, AI docs, UX catalog, and lockstep CI/release wiring |
| §22 | Pixel-first `@kerfjs/ui` CSS authoring | Shipped — `remify(<px>)` compiles against a fixed 16px baseline into standard `rem`; package CSS ships only compiled output and the UX catalog applies the same transform with HMR |
...
`DialogHeader` composes a real top toolbar: a borderless identity group keeps
the 24px icon in its 34px circle and the first title line vertically aligned
with the automatic 44px action group. The optional summary/id remains in a
separately aligned row, and `actionsLabel` supplies a localized group name.

**Shipped.** The `ui/` sibling package is a first-party, independently installable component layer that releases in version lockstep with kerfjs. Its generic primitives cover Lucide icons, toolbars, controlled segmented choices, a DOM-owned free-text and controlled token-chip searchbox, menus including `MenuActionRow` for sibling primary/trailing native controls, controlled reorderable horizontally scrolling tab bars, headers/value tables, resizable regions, a Web Awesome select adapter, banners, empty states, and loading. The generalized pane composition gives sidebars, main areas, inspectors, and dialogs unpadded toolbar/content/footer structure, 24px major content rhythm, child-owned 8px margin + 1px border + 8px padding geometry, 12px or 22px radii, and 44px rows, actions, and toolbar groups. The Select adapter preserves slotted option icons across Kerf rerenders and keys custom selected content by the controlled value. CSS defaults match Hot Sheet 2 through Web Awesome-compatible, namespaced semantic ramps; application, tone, and instance overrides remain explicit. Each browser component subpath automatically reaches the foundation, its own CSS, and styles for source-imported UI subcomponents; unused component CSS remains unreachable, while the root barrel, `unstyled` root, and Node/SSR paths stay CSS-free. The type-only `@kerfjs/ui/webawesome` subpath supplies Kerf JSX declarations for all 70 supported Web Awesome elements, synchronized against the canonical catalog and installed custom-elements manifest; custom-element registration remains an explicit side-effect subpath and Web Awesome is optional. Components keep domain state/policy in the app and eventful behavior in disposer-returning wiring. Accessibility contracts cover visible focus, decorative versus meaningful icons, sibling row-action ownership, status versus alert, native segmented choices, token-chip editing, keyboard separators and tab reorder/focus restoration, reduced motion, and forced colors. A shipped need-first decision model tells AI tools to search, reuse, compose, add a thin application adapter, or use genuinely custom semantics. Its schema-described machine-readable catalog covers every public visual/helper export, generalized layout/menu composition, and supported Web Awesome entry with imports, wiring, ownership, alternatives, variants, accessibility, exact public CSS classes, routes, and current links; props and tokens remain preferred, while composition selectors may join only cataloged public classes. Deterministic typed generation feeds the UX catalog, and automated gates reject missing exports, stale paths, invalid relationships/schema, manifest/declaration drift, missing routes, and broken links. The catalog-aware regression oracle accepts those supported selectors while historical measured manifests replay their recorded stricter boundary and retain their original scores. `ui/ux-demo/` imports the real public subpaths into a category-grouped master/detail catalog with one focused route per first-party visual component/composition, all 70 free Web Awesome 3.12 components under a collapsible ecosystem section, complete public variants, one grouped `Uses` / `Used by` selector across both sets, and shared semantic shell/component themes, while unit, consumer-bundle, and three-engine Playwright suites guard markup, behavior, CSS/JavaScript tree-shaking, registration, and wide/narrow layouts. See [`docs/21-ui-package.md`](../21-ui-package.md).
