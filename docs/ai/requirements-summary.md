...
Eight opt-in runtime warnings gated by dev mode (`isDevMode()`) + a per-warning `KERF_DEV_WARN_<NOUN>=1` env var: `…_REBUILT_LISTENERS` (Rule 4 — MutationObserver-based detection of imperative listeners discarded by the morph), `…_UNTRACKED_SIGNALS` (Rule 7 — `DevSignal` subclass firing on writes to signals with no subscribers), `…_NARROW_SET` (Rule 8 — `defineStore.set(next)` missing keys from the current state), `…_DELEGATE_IN_EFFECT` (`delegate()` inside an `effect()` body leaks listeners), `…_EACH_IN_MORPH_SKIP` (`each()` under `data-morph-skip` never updates), `…_DUPLICATE_EACH_KEYS` (duplicate `cacheKey` values), `…_VALUE_ONLY_RERENDER` (a re-render whose diff is value-only — every changed hole could have been a fine-grained binding), and `…_STALE_BINDING` (a global binding switching signal instance on the byte-equal fast path). All follow the same family contract: production short-circuit, env-var gate, default off, one-shot dedup per "owner" (mount / signal / store / hole), message ending with the canonical fix + how to silence, and no public-API surface. Lint + strict-TS + the dev-warns form a three-tier defense stack (lint at edit time, tsc at build time, dev-warns at runtime). Two members are **always-on** (not env-gated, throw not warn): the double-mount guard (§11.2.7) and — KF-340 (§11.2.8) — the dangerous-URL screen, which throws in dev and warns+drops in prod (mode via the shared `globalThis.KERF_DEV` / `NODE_ENV` probe in `src/utils/devMode.ts`).
