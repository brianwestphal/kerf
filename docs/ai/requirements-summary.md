...
| §20 | `kerfjs/router` subpath — the opt-in "postcard router": route matching + `navigate` + `delegate()` link interception + a keyed outlet | Partial — the router ships, but base-path stripping currently accepts raw prefixes (`/app` also matches `/apple`) instead of requiring a segment boundary |
...
**Overall status: Partial.** The router surface ships, but history-mode base stripping currently uses a raw string prefix: a base of `/app` also accepts and strips `/apple`. Base matching still needs a path-segment boundary check.
