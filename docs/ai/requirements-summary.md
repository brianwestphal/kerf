...
**Partial (first increment).** KF-284 adds a coverage axis orthogonal to v8 line/branch coverage: a per-behavior index (`docs/14-feature-coverage.md`) mapping each behavior — the list-reconciler state machine (`first-render ↔ granular ↔ snapshot ↔ empty-binding ↔ drift-recovery`), its *transitions*, and the core public API — to the test that would fail if it regressed. `scripts/check-feature-coverage.mjs` (`npm run check:features`, wired into `npm run check`) fails if any indexed row's guarding test (file + title) no longer resolves, so a renamed/deleted guarding test trips the gate. Rationale: two critical KF-125 reconciler bugs (select-after-delete, append-after-clear) shipped under 100% line coverage because the *transitions between states* were never asserted — line coverage is structurally blind to a missing transition. The index is **seeded** (22 rows); expanding it to every documented behavior across §1–§13 is a tracked follow-up. Complements the existing `check-doc-api-coverage` (export surface) and `check-doc-test-inventory` (test-file inventory) guards.
