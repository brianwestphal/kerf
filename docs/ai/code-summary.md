...
│   ├── utils/rowContract.ts       ← KF-103 "exactly one top-level element per row" helpers + KF-389/KF-417 namespace-aware row parsing: `parseRowTemplate(html, parent?)` returns a DocumentFragment parsed in the list parent's namespace. `foreignWrapper(parent)` picks the wrapper — `<svg>` for an SVG parent (except `<foreignObject>`, whose children are HTML) or `<math>` for a MathML parent (KF-417) — so rows re-enter foreign content the same route first render takes. Every reconcile path that creates a row node threads its `liveParent` through.
