...
│   │   ├── kf380-interaction-matrix.test.tsx ← KF-380 — the morph × fine-grained-binding × owned-row interaction matrix, tested adversarially (multi-slot lookahead shifts, conditional siblings around holes AND lists, container tag swaps, self-heal → granular, empty ↔ morph-move ↔ refill, long cross-state walks); two `.skip` pins for KF-381 (stranded owned rows duplicate when a conditional shares/shadows the list container). Analysis: docs/ai/test-gap-analysis-kf380.md
