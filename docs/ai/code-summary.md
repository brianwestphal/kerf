...
│   │   ├── demo-configs.test.ts ← guards the animated demo-capture configs + committed SVGs under site/scripts/demo-captures/ + site/public/demos/: every frame must use an explicit `cut` or `magic-move` transition (never domotion's silent crossfade default — the full-screen flash) and every committed SVG must keep `step-end` on its fv-N opacity tracks (no last-frame fade-out from the SVGO timing-function clobber)
