...
│   │   ├── demo-configs.test.ts ← guards the animated demo-capture configs + committed SVGs under site/scripts/demo-captures/ + site/public/demos/: every frame must use an explicit `cut` or `magic-move` transition (never domotion's silent crossfade default — the full-screen flash) and every committed SVG must keep `step-end` on its fv-N opacity tracks (no last-frame fade-out; domotion ≥ 0.18.0 emits `step-end` natively through SVGO, so the old fix-cut-timing post-pass is gone)
