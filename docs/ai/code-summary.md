...
│   └── utils/
│       ├── devMode.ts            ← shared dev-mode gate `isDevMode()`. Precedence: boolean `globalThis.KERF_DEV` override (read lazily, wins) → else `process.env.NODE_ENV !== 'production'`. Gates the store `get()` read-only guard; lets no-`process` (CDN import-map) consumers force prod with `globalThis.KERF_DEV = false`.
│       ├── devReadonly.ts        ← dev-only deep read-only Proxy for defineStore's `get()` snapshot. `devReadonlyProxy(obj)` throws a Rule-8 `TypeError` on set/delete/defineProperty and lazily wraps nested plain objects/arrays (deep, O(1)/access, no clone); reads (spread/JSON/keys/iteration/instanceof) are transparent. `toRaw(value)` deep-unwraps (structural-sharing) so a `set({ ...get() })`-derived object is stored plain — the internal signal never holds a Proxy. Never freezes the live object.
...
│   │   ├── devMode.internal.test.ts ← shared `isDevMode()` gate: boolean `globalThis.KERF_DEV` override wins (true/false), non-boolean ignored, else `NODE_ENV`, and no-`process` → dev ON. `*.internal.test.ts` so dist-full excludes it (imports the internal util directly).
│   │   ├── devReadonly.internal.test.ts ← dev deep read-only guard helpers: `devReadonlyProxy` wraps null-proto/nested objects + is idempotent/identity-stable; `toRaw` passes primitives/exotics through, unwraps a proxy to its raw, and deep-unwraps a spread-built object preserving structural sharing. `*.internal.test.ts` so dist-full excludes it.
