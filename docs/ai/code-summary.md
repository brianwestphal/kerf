...
│   │   ├── kf380-interaction-matrix.test.tsx ← KF-380 — the morph × fine-grained-binding × owned-row interaction matrix, tested adversarially (multi-slot lookahead shifts, conditional siblings around holes AND lists, container tag swaps, self-heal → granular, empty ↔ morph-move ↔ refill, long cross-state walks). Also the KF-381 regression home: a conditional sibling that shares/shadows the list container used to strand + duplicate owned rows; the self-heal now removes still-live stranded rows before repopulating (shape 1 + shape 2 + repeated toggles + granular-after-recovery). Analysis: docs/ai/test-gap-analysis-kf380.md
