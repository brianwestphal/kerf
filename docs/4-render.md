...
- If a keyed match (`id` / `data-key`) in the new template lines up with the preserved element, kerf will still move it to wherever the template places it (`moveBefore()` where supported, with an `insertBefore()` fallback). The attribute doesn't pin position.
...
Where the engine supports `Node.prototype.moveBefore()` (Chromium 133+, spreading), connected-row moves are performed atomically without disconnecting the node, preserving richer state such as `<iframe>` documents, playing media, and running CSS animations. The focus snapshot still runs: Firefox can keep the contenteditable focused while retargeting its live Selection (and even a cloned Range) to the list parent during `moveBefore()`. Kerf therefore records raw boundary-node references plus offsets and reconstructs the selection after the move. This is fully transparent; see [`docs/18-state-preserving-moves.md`](18-state-preserving-moves.md).
...
The same rule covers **`open` on custom elements** (any hyphenated tag). Web Awesome's popups — `wa-select`, `wa-dropdown`, `wa-details`, `wa-dialog`, `wa-drawer`, `wa-popover` — reflect their live open state to `open`, so stripping it on an unrelated re-render closed an open popup under the user (KF-900A8V, surfaced by a consumer's flaky "select closes right after opening" test). A template can still _set_ `open`; it just can't remove one the element owns. The rule lives in one helper, `src/utils/isUserAgentOwnedAttr.ts`, shared by the morph and the keyed-list attribute fast path.
...
- **Page-refresh diffs.** You hold a freshly-built `<article>` and want the live `<article>` on screen to morph to match — preserving focused input state and user-toggled `<details open>` while making ordinary attributes and children match the template. Imperative attributes absent from the template are removed unless an ownership escape hatch applies.
...
The regions must be disjoint: mounting the same element twice, or an element inside (or containing) an already-mounted tree, throws immediately — one mount per tree. Compose with plain functions that return JSX instead of nesting mounts; see §11.2.14 in `docs/11-dev-warnings.md` for the guard's details.
