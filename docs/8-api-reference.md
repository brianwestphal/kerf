...
**Initial-render failure.** If the first render throws (the render function, an `each()` row-contract violation, a binding whose first value throws), `mount()` rethrows the original error, returns no disposer, and rolls back: every effect and binding it wired is disposed, the dev listener observer is disconnected, the element gets back exactly the child nodes it had before the call, and it is no longer marked as mounted — so a retry on the same element succeeds. A throw from a later re-render leaves the mount live instead. See `docs/4-render.md` §4.7.
