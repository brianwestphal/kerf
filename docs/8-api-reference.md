...
Options ([`BindListOptions<T>`](#list-types)): `key` (stable per-row key — a `ListKey`, i.e. `string | number`), `render` (returns the row — a `MountResult` for content mode, or a `RowElement<T>` (`HTMLElement` / `{ el, update?, dispose? }`) for element mode), `tag` (content mode only), `virtualize`.
