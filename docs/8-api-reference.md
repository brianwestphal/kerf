...
- **Own the row element (element mode).** Return an `HTMLElement` (or `{ el, dispose? }`) and that element **is** the row — so the app owns its tag, class, `data-*`, and listeners, exactly how apps already build keyed rows (`<div class="ticket-row" data-id=…>`, a `<tr>` in a `<tbody>`). kerf keys / moves / reuses it, disposing your `dispose` when the row is removed or rebuilt; reactivity + cleanup inside the element are yours. A list may mix the two modes per row. (Content-mode `render` runs once extra at row creation for the mode probe, so keep `render` a pure projection — which bindList already requires.)
...
Options ([`BindListOptions<T>`](#list-types)): `key` (stable per-row key — a `ListKey`, i.e. `string | number`), `render` (returns the row — a `MountResult` for content mode, or a `RowElement` (`HTMLElement` / `{ el, dispose? }`) for element mode), `tag` (content mode only), `virtualize`.

### List types

`ListKey`, `ListSource<T>`, `RowElement`, and `BindListOptions<T>` are exported from `kerfjs/list`.
