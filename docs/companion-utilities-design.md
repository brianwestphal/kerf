...
- **Wave 3 (follow-ups from real adoption):** `kerfjs/timing` (`debounce` / `throttle` / `debouncedSignal`) — KF-481, a sixth subpath beyond the original five, surfaced as the most-repeated leftover imperative pattern (the `let timer` dance) in app code; `kerfjs/remount` (`remountOn` — force wholesale subtree replacement on a key change, the complement to morph-by-default) — KF-482; `resource.run(input, fetcher)` input threading — KF-480; `prompt()` / `form()` on `kerfjs/overlay` — KF-484. Both KF-482 and its sibling KF-483 (`kerfjs/imperative`) ship as **self-contained imperative binders** (the `bindList` / `scope` precedent) rather than render-core surgery — the maintainer chose that shape over hooking `morph.ts`, keeping Design rule 2 intact.
