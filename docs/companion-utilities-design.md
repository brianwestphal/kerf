...
Implementation is the field pattern, blessed once: `toElement → appendChild → mount(content) → wire dismissal via delegateCapture → return handle`. `close()` runs the mount disposer, removes the node, and resolves `result`. Dismissal wiring (escape key, backdrop click, outside click) is the part every app got subtly wrong (glassbox's `dismissOnOutsideClick` `alsoInside` allowlist; hotsheet's orphan-on-project-switch sweep) — centralizing it is most of the value. `overlay()` owns nothing global; a `toast()` sibling (single, replaceable, auto-dismiss) can live in the same subpath since it shares the mount+remove core. **Follow-up (KF-487):** `toast()` was rounded out from a strict subset toward what apps actually hand-roll — it now returns `{ el, dismiss }` (so callers can run entrance/exit transitions + wire an action), with `mode:'replace'` (collapse-to-latest), `variant` accent classes, and `enterClass`/`exitClass` conventions so CSS owns the animation. **Follow-up (KF-495):** `mode:'replace'` gained `collapse:'fade'|'instant'` (`'instant'` removes the prior toast synchronously — for a centered single slot where a fading-out old message would cross-fade through the incoming one), and the exit now REMOVES `enterClass` before adding `exitClass` (so a single-class fade works and `exitClass` needn't out-specify the entrance). **Follow-up (KF-497):** `handle.dismiss({ instant: true })` removes a single toast synchronously (skip the exit), and `mode:'replace'` `collapse:'instant'` force-removes a toast that's already mid-fade — so the "action button dismisses itself + shows a replacement" pattern doesn't cross-fade two messages in one centered slot. The region's active set lives on the region element (via a Symbol), keeping toast free of module-level state. **Follow-up (KF-490):** `confirm`/`prompt`/`form` gained a `render` slot option so design-system teams can bring their own markup (spread the `ok`/`cancel` + `input`/`error` wiring bags) while kerf keeps the promise/validate/focus/dismiss — closing the "use `overlay()`, skip the batteries-included dialogs" gap. It was a small change because the delegation was already attribute-driven; `render` just replaces the body and the `initialFocus` selectors moved from classes to the data-attributes. **Follow-up (KF-494):** a `choice<R>(message, actions, options?)` helper — the N-way sibling of `confirm` (one button per action resolves its value) with a `defaultValue` that **Enter** resolves (the "global Enter-to-confirm" model the raw `overlay()` engine otherwise required a held handle for). The raw `overlay()` engine stays the answer for fully bespoke keyboard/close control.

**Follow-up (KF-484):** the `window.*` dialog replacements were rounded out beyond boolean-only `confirm()` — `prompt(message, options?) → Promise<string | null>` (the one-field sibling; `window.prompt` is also a Tauri no-op) and `form(fields, options?) → Promise<Record<string, string> | null>` (the two-or-three-field case). Both reuse `overlay()`'s trap/dismiss internals, submit on Enter, and take an inline `validate` per field. The anchored `popover(anchor, content, options?)` idea noted in that ticket **shipped** as KF-485: an `overlay()` wrapper with non-modal defaults (`trap:false`, `dismiss:['outside']`, anchor auto-exempt) that positions `position:fixed` below/above the anchor (dependency-free flip-on-overflow + horizontal clamp) and repositions on scroll/resize. **Follow-up (KF-491):** popover's placement core was extracted and exported standalone — `positionAnchored(el, anchor, opts)` / `autoReposition(el, anchor, opts)` position any element with no overlay lifecycle (inline hints, floating labels) — plus a hover/focus `tooltip(anchor, content, opts)` built on them, so apps stop hand-rolling `positionDropdown` for non-overlay anchored UI.
...
- **Wave 3 (follow-ups from real adoption):** `kerfjs/timing` (`debounce` / `throttle` / `debouncedSignal`) — KF-481, a sixth subpath beyond the original five, surfaced as the most-repeated leftover imperative pattern (the `let timer` dance) in app code; `kerfjs/remount` (`remountOn` — force wholesale subtree replacement on a key change, the complement to morph-by-default) — KF-482; `kerfjs/attach` (`attach` — bind a non-kerf widget's lifecycle to one `data-morph-skip` node, teardown on removal via a MutationObserver; shipped as `kerfjs/imperative`, renamed to `kerfjs/attach` in KF-499) — KF-483; `resource.run(input, fetcher)` input threading — KF-480; `prompt()` / `form()` on `kerfjs/overlay` — KF-484. Both KF-482 and its sibling KF-483 (`kerfjs/attach`) ship as **self-contained imperative binders** (the `bindList` / `scope` precedent) rather than render-core surgery — the maintainer chose that shape over hooking `morph.ts`, keeping Design rule 2 intact.
