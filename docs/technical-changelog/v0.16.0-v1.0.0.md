...
New guards live in `src/utils/urlScreen.ts` (102 LOC) and `jsx-runtime.ts`; regression coverage now lives in `tests/unit/bindings-security.test.ts` and `tests/browser/trusted-html-bridges.spec.ts`.
