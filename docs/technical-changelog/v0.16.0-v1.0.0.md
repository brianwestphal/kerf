...
**Theme:** one new runtime tier (fine-grained signal bindings), a deep security-hardening pass across the escaping/URL surface, supply-chain hardening of the release pipeline, and a shift to official upstream benchmark numbers. No public API removals; bundle stays ~11–12 KB gzipped.
...
- Create-path overhead of row bindings leveled from **1.65× → ~1.15×** (`3e72c1d`) via allocation-free root-attr wiring.
