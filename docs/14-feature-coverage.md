...
| FC-DW11 | The ALWAYS-ON list-identity-shift warning (no env var): fires once per list when an unkeyed list's call-order id is taken over by another list, stays silent for keyed lists, for ordinary renders, and in production mode | `src/dev-list-key-warn.ts` | `tests/unit/dev-list-key-warn.internal.test.tsx` › "dev-list-key-warn (always-on identity-shift warning)" |
