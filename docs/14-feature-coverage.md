...
| FC-SV6 | SVG namespacing under `mount()`: a conditional SVG element inserted by the static-surrounds morph gets the SVG namespace (the docs/7 "mount() is enough" claim holds for surrounds); `each()` rows inside an `<svg>` root come back HTML-namespaced from every post-first-render parse — KNOWN BUG KF-389, current wrong-namespace behavior pinned asserting for granular insert, snapshot append, and structural update | `src/morph.ts`, `src/utils/rowContract.ts` | `tests/unit/kf387-seam-sweep.test.tsx` › "a conditional SVG element in the static surrounds gets the SVG namespace when the morph inserts it"; `tests/unit/kf387-seam-sweep.test.tsx` › `each() rows inside an svg root come back HTML-namespaced from every post-first-render parse` |
