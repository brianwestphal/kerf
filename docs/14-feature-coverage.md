...
| FC-T20 | List identity across a varying `each()` call count — KNOWN BUG KF-388, current behavior pinned asserting: a batched conditional-toggle + granular patch applies the patch against the WRONG list's binding (corrupt rows); an unbatched toggle keeps content correct but silently rebuilds the sibling list in both directions; a nested `each()` inside a row drifts the id counter via cache hits and rebuilds an unrelated sibling list. The pins flip when KF-388 lands | `src/each.ts`, `src/mount.ts` | `tests/unit/kf387-seam-sweep.test.tsx` › "a batched conditional-toggle + granular patch renders the WRONG list's rows"; `tests/unit/kf387-seam-sweep.test.tsx` › "a batched conditional-toggle + granular update splices the two lists' rows together"; `tests/unit/kf387-seam-sweep.test.tsx` › "an unbatched conditional each() toggle keeps content correct but silently rebuilds the sibling list"; `tests/unit/kf387-seam-sweep.test.tsx` › "a nested each() inside a row drifts the id counter with cache hits, rebuilding an unrelated sibling list" |
