...
| FC-T21 | `each()` rows × table parsing: `<tr>` rows inside an explicit `<tbody>` bind and reconcile cleanly (granular push + update); `<tr>` rows DIRECTLY under `<table>` misbind through the parser-inserted implicit tbody — KNOWN BUG KF-391, current duplicate-row behavior pinned asserting until the row-contract error lands | `src/mount.ts`, `src/utils/rowContract.ts` | `tests/unit/kf387-seam-sweep.test.tsx` › "each() of tr rows inside an explicit tbody binds and reconciles cleanly"; `tests/unit/kf387-seam-sweep.test.tsx` › "each() of tr rows directly under table misbinds through the parser-inserted tbody and duplicates rows" |
