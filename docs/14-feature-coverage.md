...
| FC-RN13d | Form-state sync × list reconcile: a dirty checkbox row follows arraySignal updates on the morph route; the attribute-only fast path does NOT sync the property — KNOWN BUG KF-390, current stale-property behavior pinned asserting for both `checked` and `value` until the fast path calls the sync | `src/utils/syncFormProp.ts`, `src/list-reconcile-fast-paths.ts` | `tests/unit/kf387-seam-sweep.test.tsx` › "a dirty checkbox row follows arraySignal updates when the diff routes through the morph path"; `tests/unit/kf387-seam-sweep.test.tsx` › "a dirty checkbox row does NOT follow an attribute-only arraySignal update (fast path skips the property sync)"; `tests/unit/kf387-seam-sweep.test.tsx` › "a dirty text input row keeps the stale user value on an attribute-only value update (fast path skips the sync)" |
