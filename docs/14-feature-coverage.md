...
| FC-T20 | List identity across a varying `each()` call count: the DATA-CORRUPTION half is fixed (KF-388 — a batched conditional-toggle + granular patch/update now renders each list's own rows), while the correct-but-LOSSY half is pinned as current behavior — an unbatched toggle keeps content correct but silently rebuilds the sibling list in both directions, and a nested `each()` inside a row drifts the id counter via cache hits, rebuilding an unrelated sibling list. Those two pins flip when KF-392 (stable list identity) lands | `src/each.ts`, `src/mount.ts` | `tests/unit/kf387-seam-sweep.test.tsx` › "a batched conditional-toggle + granular patch renders its own list's rows"; `tests/unit/kf387-seam-sweep.test.tsx` › "a batched conditional-toggle + granular update keeps the two lists' rows separate"; `tests/unit/kf387-seam-sweep.test.tsx` › "an unbatched conditional each() toggle keeps content correct but silently rebuilds the sibling list"; `tests/unit/kf387-seam-sweep.test.tsx` › "a nested each() inside a row drifts the id counter with cache hits, rebuilding an unrelated sibling list" |
