...
- `src/overlay.ts` — the `kerfjs/overlay` subpath: the blessed modal/overlay + dismiss manager (`overlay`) plus promise-based `confirm` / `prompt` / `form` / `choice` (N-way) as `window.*` replacements with a bring-your-own-markup `render` slot, non-modal anchored `popover` + `tooltip`, the standalone placement primitives `positionAnchored` / `autoReposition`, and `toast` (`{ el, dismiss }`, replace/collapse modes, CSS enter/exit transitions). Owns focus-trap / Escape-backdrop-outside dismissal / restore-focus. Structural only — ships no CSS. Shares the core with the main barrel via code-splitting. (Large — 1100+ LOC; a split is tracked.)
