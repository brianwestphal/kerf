...
- `src/timing.ts` — the `kerfjs/timing` subpath: `debounce(fn, ms)` (trailing-edge) / `throttle(fn, ms)` (leading + one trailing) — both callable with `cancel()` / `flush()` — and `debouncedSignal(source, ms)`, a read-only signal that trails `source` so it composes inside the reactive graph. `debounce` / `throttle` are dependency-free; `debouncedSignal` pulls signals only.
