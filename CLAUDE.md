...
22. `22-ui-css-authoring.md` — **shipped**: pixel-first `remify(<px>)` source-CSS authoring for `@kerfjs/ui`, compiled against a fixed 16px baseline into standard `rem` CSS for package delivery and transformed live by the UX catalog's Vite dev server; runtime props use the separate CSS-free branded `@kerfjs/ui/css-values` builders, and `List.gap` accepts finite token shorthands or complete typed lengths.
