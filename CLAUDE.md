...
22. `22-ui-css-authoring.md` — **shipped**: pixel-first `remify(<px>)` authoring for `@kerfjs/ui`, compiled against a fixed 16px baseline into standard `rem` CSS for package delivery and transformed live by the UX catalog's Vite dev server.
23. `23-app-layouts.md` — **shipped**: the opt-in `@kerfjs/ui` app/dialog layouts — a reactive `deviceClass()` device-class foundation (`@kerfjs/ui/device-class`) and four tree-shakeable whole-screen layouts (`NavStack`, `SplitView`, `Workbench`, `TabScaffold`), each declarative with a disposer-returning `wire…` helper and a companion CSS import (non-barrel subpaths). Includes the responsive presentation matrix and the AI layout-selection decision guide (`ui/docs/app-layouts.md`). UX-demo recipes + 3-engine Playwright still pending.
...
- **Coverage is a floor, not a ceiling**: meeting the enforced coverage thresholds shows code *ran*, not that every *behavior* — or every *sequence* of behaviors — is *asserted*. It is structurally blind to a **missing state transition**: a bug living in an untested interaction can sail through a green report because the individual lines still get hit by isolated, single-operation tests.
