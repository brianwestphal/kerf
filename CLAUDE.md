...
- `src/dev-row-key-warn.ts` — the always-on-when-installed missing-`data-key` row warning, moved out of `utils/rowContract.ts` so it ships in the dev chunk rather than the main bundle. Internal.
