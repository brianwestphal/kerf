...
- `src/dev-hooks.ts` — the dev-hook registry: nullable slots core calls through (`devHooks.listRebind?.(…)`) instead of importing a `dev-*` module. kerf does NOT infer dev mode — inferring it resolved to DEVELOPMENT inside production browser bundles, because bundlers substitute the bare `process.env.NODE_ENV` token and never create a `globalThis.process` object. Installation is the dev signal. Internal.
...
- `src/overlay.ts` — the `kerfjs/overlay` subpath entry + the `overlay()` core: the blessed modal/overlay + dismiss manager (`overlay`) and the non-modal anchored `popover` + `tooltip`. Owns focus-trap / Escape-backdrop-outside dismissal / restore-focus; fallback overlays arbitrate those dismissals through a document-owned open-order stack behind a private `Symbol` (no module-level mutable state). Opt-in `native: true` on the seven overlay/dialog/popover/tooltip surfaces (feature-detected) hosts a modal in a `<dialog>.showModal()` and a non-modal in a `[popover]`, falling back to the plain `<div>` where unsupported (KF-526; `docs/19`); it does not apply to toast or the standalone positioning helpers. Structural only — ships no CSS. Shares the core with the main barrel via code-splitting. Re-exports the promise-dialog helpers from `overlay-dialogs.ts`, the positioning primitives from `overlay-position.ts`, and `toast` from `overlay-toast.ts`, so the `kerfjs/overlay` surface is one entry.
- `src/overlay-dialogs.ts` — the promise-based dialog helpers for `kerfjs/overlay` (KF-530, split out of `overlay.ts`): `confirm` / `prompt` / `form` / `choice` (N-way) as `window.*` replacements, each a thin builder over `overlay()` (default markup or a bring-your-own-markup `render` slot; buttons/inputs wired via `delegate()`; resolves a promise on OK / Cancel / dismissal). Imports `overlay` back from `overlay.ts` — a call-time-only reference, so the module cycle is safe. Re-exported from `overlay.ts`. Imports `overlay` + `jsx` + `delegate`.
