...
- `src/dev.ts` — the `kerfjs/dev` subpath entry: imports every `dev-*` module and installs `DEV_HOOKS`. Consumers gate it with their own flag (`if (import.meta.env.DEV) await import('kerfjs/dev')`), which folds to false in a production build so the chunk is never emitted — worth ~4.7 KB min+gzip. Re-exports `clearDevHooks`/`installDevHooks`/`devHooks` for consumer test isolation.
