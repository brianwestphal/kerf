...
- `src/overlay.ts` — the `kerfjs/overlay` subpath entry + the `overlay()` core: the blessed modal/overlay + dismiss manager (`overlay`) and the non-modal anchored `popover` + `tooltip`. Owns focus-trap / Escape-backdrop-outside dismissal / restore-focus. Opt-in `native: true` (all surfaces, feature-detected) hosts a modal in a `<dialog>.showModal()` and a non-modal in a `[popover]`, falling back to the plain `<div>` where unsupported (KF-526; `docs/19`). Structural only — ships no CSS. Shares the core with the main barrel via code-splitting. Re-exports the promise-dialog helpers from `overlay-dialogs.ts`, the positioning primitives from `overlay-position.ts`, and `toast` from `overlay-toast.ts`, so the `kerfjs/overlay` surface is one entry.
