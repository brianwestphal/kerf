...
- `src/overlay.ts` — the `kerfjs/overlay` subpath entry + the modal-dialog core: the blessed modal/overlay + dismiss manager (`overlay`) plus promise-based `confirm` / `prompt` / `form` / `choice` (N-way) as `window.*` replacements with a bring-your-own-markup `render` slot, and non-modal anchored `popover` + `tooltip`. Owns focus-trap / Escape-backdrop-outside dismissal / restore-focus. Opt-in `native: true` (all surfaces, feature-detected) hosts a modal in a `<dialog>.showModal()` and a non-modal in a `[popover]`, falling back to the plain `<div>` where unsupported (KF-526; `docs/19`). Structural only — ships no CSS. Shares the core with the main barrel via code-splitting. Re-exports the positioning primitives from `overlay-position.ts` and `toast` from `overlay-toast.ts`.
