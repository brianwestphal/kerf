...
- `src/dev-binding-warn.ts` / `src/dev-delegate-warn.ts` / `src/dev-each-warn.ts` / `src/dev-listener-warn.ts` / `src/dev-rerender-warn.ts` / `src/dev-store-warn.ts` — the opt-in `KERF_DEV_WARN_*` warning family (design rules in `docs/11-dev-warnings.md`): stale fine-grained binding on the byte-equal fast path (`…_STALE_BINDING`), `delegate()` inside an `effect()` body (`…_DELEGATE_IN_EFFECT`), `each()` under `data-morph-skip` + duplicate `cacheKey`s (`…_EACH_IN_MORPH_SKIP` / `…_DUPLICATE_EACH_KEYS`), morph-rebuilt nodes carrying imperative listeners (`…_REBUILT_LISTENERS`), value-only re-renders that could have been bindings (`…_VALUE_ONLY_RERENDER`), and narrow `store.set()` calls (`…_NARROW_SET`). All dev-only, gated through `utils/devMode.ts`; production behavior unchanged. Internal.
...
11. `11-dev-warnings.md` — the opt-in `KERF_DEV_WARN_*` env-gated dev-warn family (rebuilt listeners, untracked signals, narrow store sets, delegate-in-effect, each-in-morph-skip, duplicate keys, value-only re-renders, stale bindings) and the rules each new warning must follow.
