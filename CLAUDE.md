...
- `src/scope.ts` — the `kerfjs/scope` subpath: tie disposers to a DOM element's lifetime so append-heavy UIs stop leaking detached-but-subscribed effects/listeners. `disposeScope(el)` → a WeakMap-keyed accumulating scope (`add` / `mount` / `effect` / `delegate` wrappers + idempotent `dispose()`); `disposeSubtree(root)` sweeps a subtree; `observeRemovals(root)` installs one `MutationObserver` that auto-disposes on removal. No module-level mutable state.
