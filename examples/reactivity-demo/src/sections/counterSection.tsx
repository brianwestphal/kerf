...
export function mountCounter(root: HTMLElement): void {
    ...
    <div className="demo-card">
      <h2>
        ...
        <span className="demo-tag">
          single signal • Tier 1 click delegation
        </span>
      </h2>
      <div className="demo-row">
        <button type="button" {...ACTIONS.dec.attrs} className="demo-btn">
          −
        </button>
        <output className="demo-counter-value">{count.value}</output>
        <button type="button" {...ACTIONS.inc.attrs} className="demo-btn">
          +
        </button>
        <button
          type="button"
  ...
  delegate(root, 'click', ACTIONS.inc.selector, () => {
    count.value += 1;
  });
  delegate(root, 'click', ACTIONS.dec.selector, () => {
    count.value -= 1;
  });
