...
test('the Loading inspector recipe composes placeholder chrome and swaps to loaded', async ({ page, browserName }) => {
  ...
  if (browserName === 'chromium') await inspector.screenshot({ path: 'test-results/recipe-loading-inspector.png' });
  ...
  await expect(inspector.locator('.kui-value-table__row[data-placeholder="true"]')).toHaveCount(0);
