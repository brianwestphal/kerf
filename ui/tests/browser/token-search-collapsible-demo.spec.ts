...
test('the TokenSearchField demo shows and drives the collapsible state', async ({ page, browserName }) => {
  await page.setViewportSize({ width: 1100, height: 700 });
  await page.goto('/?component=token-search-field');

  const field = page.locator('.token-search-demo__collapsible [data-component="token-search-field"]');
  ...
  if (browserName === 'chromium') await page.locator('.token-search-demo__collapsible').screenshot({ path: 'test-results/token-search-collapsed.png' });
  ...
  if (browserName === 'chromium') await page.locator('.token-search-demo__collapsible').screenshot({ path: 'test-results/token-search-expanded.png' });
