...
const headerGeometry = async (page: Page) =>
  page.evaluate(() => {
    ...
    const subtitleStyle = window.getComputedStyle(
      document.querySelector<HTMLElement>(".catalog-brand__subtitle")!,
    );
    const collapse = rect(
      '.catalog-brand [aria-label="Collapse component catalog"]',
    );
...
test("uses the Kerf identity and relocates sidebar restore into the detail toolbar", async ({
  ...
  await sidebar
    .getByRole("button", { name: "Collapse component catalog" })
    .click();
  await expect(shell).toHaveAttribute("data-sidebar-collapsed", "true");
  await expect(sidebar).toBeHidden();
  await expect(
    sidebar.getByRole("button", { name: "Collapse component catalog" }),
  ).toHaveCount(0);

  const restore = detail.getByRole("button", {
    name: "Expand component catalog",
  });
  ...
  expect(
    await restore.evaluate(
      (element) =>
        element ===
        document.querySelector(".catalog-header .kui-toolbar__leading button"),
    ),
  ).toBe(true);
  await expect
  ...
  await sidebar
    ...
    .click();
  await expect(
    detail.getByRole("button", { name: "Expand component catalog" }),
  ).toBeVisible();
  await expect
    .poll(() =>
      page.evaluate(
