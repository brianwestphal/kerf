...
import { CollapsiblePanel, CollapsiblePanelToggle, collapsiblePanelToggleIcon } from '../../src/collapsible-panel.js';
...
describe('CollapsiblePanel', () => {
  it('renders a docked panel with side, collapsed, label, and a sliding content wrapper', () => {
    const open = html(CollapsiblePanel({ id: 'nav', side: 'left', label: 'Navigator', size: 320, children: raw('<p>items</p>') }));
    ...
    const collapsed = html(CollapsiblePanel({ id: 'nav', side: 'left', collapsed: true, children: raw('<p>items</p>') }));
  ...
  it('uses the height custom property for a bottom drawer', () => {
    const drawer = html(CollapsiblePanel({ id: 'console', side: 'bottom', size: 240, children: raw('<x/>') }));
  ...
  it('picks the standard per-side collapse/expand glyph', () => {
    expect(collapsiblePanelToggleIcon('left', false).name).toBe('panel-left-close');
    expect(collapsiblePanelToggleIcon('left', true).name).toBe('panel-left-open');
    expect(collapsiblePanelToggleIcon('right', false).name).toBe('panel-right-close');
    expect(collapsiblePanelToggleIcon('right', true).name).toBe('panel-right-open');
    expect(collapsiblePanelToggleIcon('bottom', false).name).toBe('panel-bottom-close');
    expect(collapsiblePanelToggleIcon('bottom', true).name).toBe('panel-bottom-open');
  });

  it('renders a standard toggle button with the action, aria-expanded, and target', () => {
    const collapse = html(CollapsiblePanelToggle({ side: 'left', collapsed: false, action: 'toggle-nav', panelId: 'nav' }));
    ...
    const expand = html(CollapsiblePanelToggle({ side: 'left', collapsed: true, action: 'toggle-nav', label: 'Show navigator' }));
