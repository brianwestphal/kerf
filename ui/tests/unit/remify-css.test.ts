...
describe('remify CSS authoring transform', () => {
  it('converts integer, fractional, negative, and zero pixel literals exactly', () => {
    expect(
      transformRemifyValue(
        'remify(17px) remify(18px) remify(40.4px) remify(-2px) remify(0px)',
      ),
    ).toBe('1.0625rem 1.125rem 2.525rem -0.125rem 0rem');
  });
  ...
  it('does not transform quoted or commented examples', () => {
    expect(
      transformRemifyValue('"remify(16px)" /* remify(32px) */ remify(48px)'),
    ).toBe('"remify(16px)" /* remify(32px) */ 3rem');
  });

  it('rejects expressions and non-pixel arguments with a source location', async () => {
    await expect(
      postcss([remifyCss()]).process('.bad { width: remify(var(--size)); }', {
        from: 'fixture.css',
      }),
    ).rejects.toThrow('remify() accepts one numeric px literal');
    await expect(
      postcss([remifyCss()]).process('.bad { width: remify(2em); }', {
        from: 'fixture.css',
      }),
