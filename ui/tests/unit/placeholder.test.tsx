...
describe('Skeleton primitive', () => {
  ...
  it('applies width, height, and radius as inline style', () => {
    const html = asHtml(Skeleton({ width: 'remify(120px)', height: '1.5em', radius: '2px' }));
...
describe('component placeholder mode', () => {
  it('ToolbarText replaces its text with a skeleton', () => {
    const html = asHtml(ToolbarText({ text: 'Ticket title', size: 'xlarge', placeholder: true }));
  ...
  it('ValueTableRow keeps the field label and skeletons the value', () => {
    const html = asHtml(ValueTableRow({ label: 'Owner', value: 'Jordan', placeholder: true }));
    expect(html).toContain('data-placeholder="true"');
  ...
  it('PanelHeader keeps the icon and skeletons the title and summary', () => {
    const html = asHtml(PanelHeader({ title: 'Ticket', titleId: 't', summary: 'A concise summary', summaryId: 's', icon, placeholder: true }));
    expect(html).toContain('data-placeholder="true"');
  ...
  it('AppTab skeletons the name, disables its buttons, and is not draggable', () => {
    const html = asHtml(AppTab({ id: 't1', name: 'Overview', draggable: true, selectAction: 'pick', placeholder: true }));
    expect(html).toContain('data-placeholder="true"');
  ...
  it('SegmentedControl disables every segment and skeletons their content', () => {
    const html = asHtml(SegmentedControl({ id: 'view', label: 'View', value: 'list', action: 'select-view', choices: [
      { value: 'list', label: 'List', content: <LucideIcon icon={List} name="list" /> },
      { value: 'columns', label: 'Columns', content: <LucideIcon icon={Columns3} name="columns-3" /> },
    ], placeholder: true }));
    expect(html).toContain('data-placeholder="true"');
  ...
  it('StateBanner skeletons the title and detail, keeping the icon', () => {
    const html = asHtml(StateBanner({ title: 'Syncing', detail: 'Fetching the latest', icon, placeholder: true }));
    expect(html).toContain('data-placeholder="true"');
  ...
  it('Select renders a static control box instead of the interactive wa-select', () => {
    const html = asHtml(Select({ name: 'status', value: 'open', label: 'Status', choices: [{ value: 'open', label: 'Open' }], placeholder: true }));
  ...
  it('Select placeholder omits the label element when no label is given', () => {
    const html = asHtml(Select({ name: 'status', value: 'open', ariaLabel: 'Status', choices: [{ value: 'open', label: 'Open' }], placeholder: true }));
    expect(html).toContain('kui-select--placeholder');
  ...
  it('Select still emits its placeholder hint text when not loading', () => {
    const html = asHtml(Select({ name: 'status', value: '', ariaLabel: 'Status', placeholderText: 'Choose a status', choices: [{ value: 'open', label: 'Open' }] }));
