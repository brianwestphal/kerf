...
export function MenuHeader({ label, action, actionLabel, actionIcon, actionDisabled = false, disabledReason, expanded, toggle = false }: MenuHeaderProps) {
  if (toggle) {
    return <button type="button" class="kui-menu-header kui-menu-header--toggle" data-component="menu-header" data-action={action} aria-expanded={String(Boolean(expanded))}><span>{label}</span>{actionIcon && <span class="kui-menu-header__action-layer">{actionIcon}</span>}</button>;
  }
