...
const webAwesomeOutputPath = resolve(
  root,
...
const entries = [
  ...
  ['@kerfjs/ui/sunken-panel', 'dist/sunken-panel.d.ts'],
...
const webAwesomeDeclaration = await readFile(
  resolve(root, 'dist/webawesome.d.ts'),
  'utf8',
);
const webAwesomeBody =
...
const webAwesomeDigest = createHash('sha256')
  .update(webAwesomeBody)
  .digest('hex');
if (check) {
  ...
  if (existing !== body)
    throw new Error(
      'ai/public-api-signatures-v1.md is stale; run npm run ai:signatures:sync',
    );
  if (existingWebAwesome !== webAwesomeBody)
    throw new Error(
      'ai/webawesome-jsx-signatures-v1.md is stale; run npm run ai:signatures:sync',
    );
  console.log(
    `[sync-ai-public-signatures] OK — ${entries.length} pinned API subpaths (${digest}) and Web Awesome JSX declarations (${webAwesomeDigest})`,
  );
} else {
  await Promise.all([
  ...
  console.log(
    `[sync-ai-public-signatures] wrote API (${digest}) and Web Awesome JSX (${webAwesomeDigest}) signature contexts`,
  );
}
