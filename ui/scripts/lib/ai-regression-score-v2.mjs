...
export function scoreAiRegressionV2(caseDefinition, response, catalog, options = {}) {
  ...
  const sourceIndex = base.checks.findIndex(({ code }) => code.startsWith('duplicate:'));
  const index = sourceIndex < 0 ? base.checks.length : sourceIndex;
  const checks = [...base.checks.slice(0, index), ...wiringChecks, ...base.checks.slice(index)];
  const dimensions = recompute(checks);
  return { caseId: base.caseId, pass: checks.every((check) => check.pass), dimensions, checks };
}
