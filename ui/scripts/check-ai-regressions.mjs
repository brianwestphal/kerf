...
const [corpus, schema, conditions, conditionsSchema, conditionsV2, conditionsV2Schema, responseSchema, runSchema, runV2Schema, suiteV2, compileSchema, overrides, expected, catalog, packageJson] = await Promise.all([
...
const expectedV2RevisedContextSha256 = 'b3c7e4cf215ac83afc9f014df81f0547cc629b504657da9bc19eeed34061eb98';
...
if (runV2Schema.properties?.schemaVersion?.const !== 2 || runV2Schema.additionalProperties !== false) fail('suite-v2 run schema must pin version 2 and reject unknown root fields');
...
if (JSON.stringify(conditionsV2.conditions?.map(({ id }) => id)) !== JSON.stringify(expectedConditions)) fail('suite-v2 conditions must preserve the three stable condition ids');
...
if (conditionsV2.schemaVersion !== 2 || conditionsV2Schema.properties?.schemaVersion?.const !== 2) fail('suite-v2 conditions and schema must pin version 2');
if (suiteV2.schemaVersion !== 2 || suiteV2.id !== 'kerf-ui-authoring-v2') fail('suite-v2 descriptor must pin its version and id');
for (const [key, expectedPath] of Object.entries({
  ...
  revisedContextSnapshot: 'ai-regressions/contexts/revised-recipes-catalog-v2.snapshot.json',
  ...
  if (suiteV2[key] !== expectedPath) fail(`suite-v2 descriptor has stale ${key}`);
}
const revisedV2 = conditionsV2.conditions.find(({ id }) => id === 'revised-recipes-catalog');
for (const required of ['ai/public-api-signatures-v1.md', 'docs/examples/command-palette-adapter.tsx', 'ux-demo/recipes/mount-recipe.ts']) {
  if (!revisedV2.sourcePaths.includes(required)) fail(`suite-v2 revised context is missing ${required}`);
}
if (AI_REGRESSION_V2_CONTEXT_SNAPSHOTS.get('revised-recipes-catalog') !== 'ai-regressions/contexts/revised-recipes-catalog-v2.snapshot.json') fail('v2 revised context must resolve through its checked-in snapshot');
for (const condition of conditionsV2.conditions) {
  const snapshotPath = AI_REGRESSION_V2_CONTEXT_SNAPSHOTS.get(condition.id);
  const first = await buildAiRegressionContext(root, condition, { snapshotPath });
  const second = await buildAiRegressionContext(root, condition, { snapshotPath });
  if (first.sha256 !== second.sha256 || first.text !== second.text) fail(`suite-v2 ${condition.id} context assembly is nondeterministic`);
  if (condition.id === 'revised-recipes-catalog' && first.sha256 !== expectedV2RevisedContextSha256) fail('suite-v2 revised context snapshot changed');
  if (condition.expectedSha256 && first.sources[0]?.sha256 !== condition.expectedSha256) fail(`suite-v2 ${condition.id} frozen source content changed`);
}
