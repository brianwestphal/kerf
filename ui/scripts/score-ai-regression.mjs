...
if (!caseId || !responsePath) {
  console.error('Usage: npm run ai:regressions:score -- --suite <1|2> --case <id> --response <response.json>');
  process.exit(2);
}
...
const [corpus, catalog, response, overrides] = await Promise.all([
  ...
  suiteVersion === 2 ? readFile(resolve(root, 'ai-regressions/corpus-v2-overrides.json'), 'utf8').then(JSON.parse) : Promise.resolve(null),
]);
const testCase = corpus.cases.find(({ id }) => id === caseId);
if (!testCase) throw new Error(`Unknown AI regression case: ${caseId}`);
const definition = suiteVersion === 2 ? { ...testCase, ...overrides.cases[testCase.id] } : testCase;
const result = suiteVersion === 2 ? scoreAiRegressionV2(definition, response, catalog) : scoreAiRegression(definition, response, catalog);
