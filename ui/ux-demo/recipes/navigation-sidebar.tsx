...
export const createRecipe: RecipeFactory = (announce) => {
  ...
  const render = () => <aside class="kui-recipe recipe-sidebar kui-recipe__surface kui-layout" data-recipe="recipe-navigation-sidebar">
    <div class="recipe-sidebar__body kui-sidebar kui-scroll-owner">
      ...
      <p class="kui-recipe__ownership">The recipe owns one highlight rail, one content rail, icon slots, and action targets. The app owns routes, permissions, labels, and disclosure state.</p>
    </div>
    <footer class="recipe-sidebar__footer"><Toolbar className="kui-sidebar-toolbar" label="Sidebar actions" divider={false} leading={<ToolbarControlGroup appearance="borderless" single><button type="button" aria-label="Add project" data-action="recipe-action" data-recipe-command="add"><LucideIcon icon={Plus} name="plus" /></button></ToolbarControlGroup>} trailing={<ToolbarControlGroup appearance="borderless" single><button type="button" aria-label="Workspace settings" data-action="recipe-action" data-recipe-command="settings"><LucideIcon icon={Settings} name="settings" /></button></ToolbarControlGroup>} /></footer>
  </aside>;
