...
| Sidebar/menu composition | Navigation needs distinct interaction and content rails, optional icon slots, and full-size row/action targets. | Do not stack wrapper padding, align text to a decorative border, or shrink targets to icon size. Use ordinary layout for a different navigation hierarchy or reading width. | Compose `MenuHeader` and `MenuItem`; apply `.kui-sidebar`, `.kui-sidebar-section`, `.kui-sidebar-surface`, and `.kui-sidebar-toolbar` where applicable. | Information architecture, responsive drawer/shell behavior, and token overrides. | `@kerfjs/ui/sidebar.css` | [Sidebar alignment](../README.md#sidebar-alignment) |
...
Correct: let one sidebar composition own the 10px interaction rail, 20px
content rail, 24px icon slots, and 44px targets.
