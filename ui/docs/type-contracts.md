...
| `KUI-T013` | CSS-valued props preserve property grammar: `List.flex` uses `CssFlex`, `Skeleton` dimensions use `CssSize`/`CssLength`, and choice icons use `CssForegroundColor` (foreground tokens via `uiColor()` or `foregroundColorVar()`; fill aliases such as `uiColor('success')` are rejected); row declaration strings are absent. |
...
- Replace raw `List.flex` shorthands with `flex(grow, shrink, basis)` or a finite
  ...
  `uiColor('success-on-quiet')` or an app-owned `foregroundColorVar()`.
  `SelectChoice.color` rejects fill aliases (`uiColor('success')`) and
  unqualified `colorVar()` values because a fill paints a nearly invisible
  icon.
