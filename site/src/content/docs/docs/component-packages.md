---
title: 'Building reusable component packages'
description: 'Building and publishing reusable kerf components as npm packages.'
---

kerf has no component runtime — a "component" is just a function that takes props
and returns [`SafeHtml`](/kerf/docs/jsx/). That makes shipping reusable components
...
to set up and publish the package.

## Quick start: scaffold with `create-kerf-component`
...
when it sees a function-valued tag (`if (typeof tag === 'function') return tag(props)`),
...
export interface ButtonProps {
  label: string;
  /** A delegation hook, NOT an inline handler — see Events and cleanup. */
  action: string;
...
outside it (see State: the one thing to get right).

## State: the one thing to get right
...
1. **Markup + delegation (preferred for most components).** The component emits
   ...
   `mount()` root with [`delegate()`](/kerf/docs/events/), which returns a
...
2. **Imperative widget (for wrapping third-party libraries).** When the component
   ...
   host marked [`data-morph-skip`](/kerf/docs/render/) and expose a create/dispose pair:
   ...
   See the [render doc](/kerf/docs/render/) for the full `data-morph-skip` /
...
break across the boundary. Keep kerfjs external.
...
A component package builds with `tsup`, emits ESM + `.d.ts`, and publishes with
npm provenance from a git tag, gated on an npm Trusted-Publisher environment.
There is no npm org/scope requirement — publish unscoped.
