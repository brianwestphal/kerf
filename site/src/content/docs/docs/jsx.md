...
Anything not in the alias table is passed through verbatim. So `data-action`, `aria-label`, `data-key` all work as expected (JSX-to-HTML uses the literal attribute name) — as long as the name is well-formed (see [Attribute name validation](#attribute-name-validation)).
...
### HTML-bearing attributes (`srcdoc`) are not URLs
...
It takes a `SafeHtml` (JSX or the `html` tagged template) or a raw string, and an optional `{ doctype }` (default `'html'`). See the [API reference](/kerf/api/).

## Typed JSX intrinsic elements
...
**Where the types come from.** Names, value sets, and per-element membership are taken from the WHATWG HTML Living Standard and SVG 2, with MDN as a readable index into them — not from another framework's table. That distinction is load-bearing: other tables describe a _property_ surface (`HTMLElement.draggable: boolean`), while kerf emits _content attributes_ into an HTML string, and the two disagree in exactly the places that cause silent bugs (see [Boolean attributes](#boolean-attributes) on enumerated attributes). Coverage is focused rather than exhaustive — a missing attribute is a gap to fill, not a verdict that it's invalid; add it via declaration merging until it lands upstream. There are a handful of deliberate departures from the spec — lowercase aliases alongside the camelCase forms, `contentEditable="inherit"`, and the obsolete presentational attributes kept as `@deprecated`.
...
## Tagged templates — `kerfjs/html` (no build step at all)
...
- **Text/child holes** follow the [Children](#children) rules exactly: strings are HTML-escaped, numbers stringify, `null` / `undefined` / booleans render nothing, arrays join, `SafeHtml` (nested `html\`\``, `raw()`, and `each()` list segments) passes through — so a list hole is owned by the keyed reconciler just like in JSX. A signal/`computed` handed in _itself_ binds fine-grained (see the API reference and the reactivity doc); DOM nodes and other unsupported types throw the same errors.
- **Attribute holes** follow the [Boolean attributes](#boolean-attributes) rules exactly: `true` renders the bare attribute, `false` / nullish omit it, `SafeHtml` values bypass the URL screen, plain strings are escaped and screened for dangerous URL schemes, `on*` attributes and malformed names are rejected, and a signal/`computed` binds the attribute fine-grained.

Two authoring differences from JSX:

1. **No camelCase aliases.** Template authors write real HTML attribute names — `class`, `for`, `tabindex`, `stroke-width` — not `className` / `htmlFor`. The alias table does not apply; an attribute name passes through verbatim.
...
### Loading kerf from a CDN (no install)
