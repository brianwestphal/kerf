...
- Not a router — _the core_ is not. An opt-in, tree-shakeable `kerfjs/router` subpath (the "postcard router": route matching + `navigate` + link interception + a keyed outlet, and deliberately nothing more) exists for apps that want one, and adds nothing to the main barrel until imported. Not a build tool. Not an SSR framework (though `SafeHtml.toString()` works server-side if you want it).
...
- [Reactivity](/kerf/docs/reactivity/) — signals primitive.
- [Stores](/kerf/docs/stores/) — composable testable stores.
- [Render](/kerf/docs/render/) — mount, segments, the native diff, and the list reconciler.
- [Event delegation](/kerf/docs/events/) — Tier 1 / Tier 2 / Tier 3.
- [JSX runtime](/kerf/docs/jsx/) — JSX → HTML strings, server-side use.
- [SVG](/kerf/docs/svg/) — namespace handling and the `toElement()` escape hatch.
- [API reference](/kerf/api/) — every export, every option.
- [Live demo](/kerf/demo/) — the GitHub Pages deploy of the reactivity demo.
- [Migrating](/kerf/migrating/) — the coming-from-React/Vue/Lit/… comparison hub.
- [Dev-mode warnings](/kerf/docs/dev-warnings/) — the opt-in `KERF_DEV_WARN_*` family.
- [Component packages](/kerf/docs/component-packages/) — publishing reusable kerf components to npm.
- [No-build example](/kerf/examples/complete/live-poll/) — the served-as-source `live-poll` app (importmap + `html` tagged template).
