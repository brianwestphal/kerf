...
Everything above is the **coarse** update path: a signal change re-runs `render()`, then kerf diffs down to the changed nodes. For a hot hole driven by an external signal (a `selectedId` flipping one row's class), you can opt that hole into a **fine-grained binding** — pass the signal itself into the attribute/text and the update skips both the re-render and the reconcile, touching only that node. See [`docs/2-reactivity.md`](/kerf/docs/reactivity/) §2.9.
