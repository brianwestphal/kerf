...
Kerf sits in the keyed-framework cluster with Solid and Vue on the operations that exercise its smallest-cut path. Solid wins the compiler-driven `select row` / `partial update` benchmarks; kerf's general-purpose reconciler trades that gap for a 6.6 KB runtime, no compiler, and no per-component machinery.
