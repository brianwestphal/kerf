...
hero:
  ...
  tagline: |
    ...
    6.5 KB. No virtual DOM. No compiler. No magic.
...
Kerf sits in the keyed-framework cluster with Solid and Vue on the operations that exercise its smallest-cut path. Solid wins the compiler-driven `select row` / `partial update` benchmarks; kerf's general-purpose reconciler trades that gap for a 6.5 KB runtime, no compiler, and no per-component machinery.

## Why Kerf

<CardGrid>
  <Card title="Built for AI-assisted coding" icon="seti:html">
    Tiny public surface (15 exports), no compiler magic, no hidden lifecycle. An LLM holds the framework in context and predicts behavior — your AI agent generates code that works the first time.
  </Card>
  <Card title="Smallest cut" icon="approve-check">
    6.5 KB gzipped including signals. Fine-grained reactivity re-runs only what changed; the diff touches only the DOM nodes that differ.
  </Card>
