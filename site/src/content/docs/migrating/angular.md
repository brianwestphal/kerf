...
**Kerf** is a ~12 KB reactive runtime — signals, a DOM morph, JSX, and a delegation-based event system. It has no router, no forms library, no HTTP wrapper, no DI, no CLI, no testing framework. You bring those (or don't, if your app doesn't need them).
