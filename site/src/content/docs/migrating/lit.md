...
Lit (~6 KB) is lighter than kerf (~12 KB), so this isn't a "size savings" migration — it's a "what shape do I want my code to be" decision. The Lit way is web components + tagged templates + Shadow DOM. The kerf way is module-scoped signals + JSX + light DOM. The side-by-side below makes the trade concrete.
