...
**Not ready for a full rewrite?** You don't need one. kerf coexists with React/Vue/Svelte on the same page — see [Adopting kerf incrementally](/kerf/migrating/incremental/) to migrate one island at a time.
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="vanjs → kerf is the smallest conceptual jump. Both use signals and update the DOM directly. The differences are JSX vs hyperscript, structured morph vs manual patching, and how keyed lists reconcile. kerf is heavier (~11 KB vs ~1.6 KB for vanjs-core)."
  ...
  <LinkCard
    ...
    description="Angular is a full framework with router/forms/DI; kerf is an ~11 KB runtime. When Angular stays right, and when kerf supplants it."
...
3. **Section by section** — the same todo list translated section by section (state, render, events, list).
