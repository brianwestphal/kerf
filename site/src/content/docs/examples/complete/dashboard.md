...
- **`each()` at scale.** The ticker table runs through the keyed list reconciler. Per-row memoization by identity _plus_ an optional memo key (`\`${id}-${price}\``) means a row whose price didn't change skips JSX evaluation, string-building, _and_ the morph walk. Only the ~60 rows that actually moved get touched.
