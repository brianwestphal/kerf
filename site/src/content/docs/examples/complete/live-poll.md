...
mount(root, () => {
  ...
  return html` <div class="poll">
    <ul class="opts">
      ${each(
        OPTIONS,
        (o) =>
          html`<li class="opt" data-key="${o.id}">
            <button class="opt-btn" data-vote="${o.id}">
              <span class="opt-label">${o.label}</span>
              <span class="opt-count">${votes.get(o.id)}</span>
              <span class="opt-track"
                ><span class="opt-bar" style="${bars.get(o.id)}"></span
              ></span>
            </button>
          </li>`,
      )}
    ...
    <div class="poll-foot">
      <b data-total>${total}</b> votes · renders: <b data-renders>${renders}</b>
    </div>
  </div>`;
});
