...
**What to look at:** the `<canvas>` is mounted _once_ outside of kerf and runs its own `requestAnimationFrame` loop. The surrounding counter re-renders on every click, but the canvas isn't even visited by the diff — its imperative drawing continues uninterrupted.
