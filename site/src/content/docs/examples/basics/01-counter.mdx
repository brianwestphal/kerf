...
**What to look at:** the render fn reads `count.value` _inside_ the closure — that's how the dependency is tracked. Reading it outside (e.g. into a `const` first) would not trigger re-renders.
