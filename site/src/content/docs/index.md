...
template: splash
hero:
  title: Reactive UI that touches only the bytes that changed.
  tagline: |
    The smallest cut.
...
Official numbers from the [krausest js-framework-benchmark](https://krausest.github.io/js-framework-benchmark/current.html) — kerf is a listed entry, so it's measured on the same reference machine as every competitor in the same run — see `bench/results.md` for the full table and methodology. Kerf sits in the same cluster as Vue, vanjs, and Lit on most operations. Solid wins the compiler-driven `select row` and `partial update` benchmarks; kerf's general-purpose runtime trades that gap for an ~11 KB bundle, no compiler step, and a smaller public API.
