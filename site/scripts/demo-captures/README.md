...
- **Cut frames hold-then-snap; `step-end` is now handled upstream.** domotion
  ...
  Re-verified on domotion **0.30.1** (the current pin): a
  `cut` still emits `animation: fv-N <t>s step-end infinite`, so the tripwire
  holds. Releases 0.29–0.30 add selectable/real-text output, system-font text
  mode, and opt-in nested-SVG flattening; none changes the cut schema or timing.
